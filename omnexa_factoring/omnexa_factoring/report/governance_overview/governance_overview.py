...
def execute(filters=None):
	...
	pol_total = frappe.db.count("Factoring Policy Version")
	pending = frappe.db.count("Factoring Policy Version", {{"status": "PENDING_APPROVAL"}})
	approved = frappe.db.count("Factoring Policy Version", {{"status": "APPROVED"}})
	rejected = frappe.db.count("Factoring Policy Version", {{"status": "REJECTED"}})
	snaps = frappe.db.count("Factoring Audit Snapshot")
