...
def policy_query_conditions(user=None):
	...
	return f"(`tabFactoring Policy Version`.maker = {esc} or `tabFactoring Policy Version`.checker = {esc} or `tabFactoring Policy Version`.rejector = {esc})"
...
def snapshot_query_conditions(user=None):
	...
	if _is_privileged(user):
		return ""
	esc = frappe.db.escape(user)
	return f"`tabFactoring Audit Snapshot`.actor = {esc}"
